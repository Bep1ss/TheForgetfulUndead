...
public class Projectile : MonoBehaviour
{
    ...
    [Header("Blood Impact Particle System")]
    [Tooltip("Particle system to instantiate when colliding with Blood tagged object")]
    public ParticleSystem bloodImpactParticleSystem;
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            Debug.Log(":(");
            // Instantiate the blood impact particle system
            Instantiate(bloodImpactParticleSystem, transform.position, Quaternion.LookRotation(collision.contacts[0].normal));
        ...
        {
            // Instantiate random impact prefab from array
            Instantiate(dirtImpactPrefabs[Random.Range(0, dirtImpactPrefabs.Length)], transform.position,
                Quaternion.LookRotation(collision.contacts[0].normal));
            // Destroy bullet object
            Destroy(gameObject);
        }
        ...
        {
            // Instantiate random impact prefab from array
            Instantiate(concreteImpactPrefabs[Random.Range(0, concreteImpactPrefabs.Length)], transform.position,
                Quaternion.LookRotation(collision.contacts[0].normal));
            // Destroy bullet object
            Destroy(gameObject);
        }
